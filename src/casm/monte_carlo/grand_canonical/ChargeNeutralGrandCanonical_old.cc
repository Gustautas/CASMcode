...
namespace CASM {
  ...
  void ChargeNeutralGrandCanonical::set_state(const GrandCanonicalConditions &new_conditions,
    ...
    const ChargeNeutralGrandCanonical::EventType &ChargeNeutralGrandCanonical::propose(){
        ...
        do{
          ...
          // first and second representing Na/Va and third representing Mn/Ti
        ...
        // Gustas: creating pairs tuples
        // Moving from pairs to tuples, changed mutating_sites, shanget sublats, changed current_occupants, new_occupants
  ...
  void ChargeNeutralGrandCanonical::_print_correlations(const Eigen::VectorXd &corr,
	...
	void ChargeNeutralGrandCanonical::_update_deltas(EventType &event, 
        ...
        // Site 1
        // ---- set OccMod --------------
        event.occupational_change().first.set(std::get<0>(mutating_sites), std::get<0>(sublats), new_occs.first);

        // ---- set dspecies --------------
        for(int i = 0; i < event.dN().first.size(); ++i) {
          event.set_dN(i, 0);
        }
        Index curr_species_1 = m_site_swaps.sublat_to_mol()[std::get<0>(sublats)][curr_occs.first];
        Index new_species_1 = m_site_swaps.sublat_to_mol()[std::get<0>(sublats)][new_occs.first];
        event.set_dN(curr_species_1, -1);
        event.set_dN(new_species_1, 1);

        // ---- set dcorr --------------
        _set_dCorr(event, std::get<0>(mutating_sites), std::get<0>(sublats), curr_occs.first, new_occs.first, m_use_deltas, m_all_correlations); // Zeyu: Shall we rewrite _set_dCorr?
        ...
        // Site 2
        // ---- set OccMod --------------
        event.occupational_change().second.set(std::get<1>(mutating_sites), std::get<1>(sublats), new_occs.second);
        // ---- set dspecies --------------
        for(int i = 0; i < event.dN().first.size(); ++i) {
          event.set_dN(i, 0);
        }
        Index curr_species_2 = m_site_swaps.sublat_to_mol()[std::get<1>(sublats)][curr_occs.second];
        Index new_species_2 = m_site_swaps.sublat_to_mol()[std::get<1>(sublats)][new_occs.second];
        event.set_dN(curr_species_2, -1);
        event.set_dN(new_species_2, 1);
        // ---- set dcorr --------------
        _set_dCorr(event, std::get<1>(mutating_sites), std::get<1>(sublats), curr_occs.second, new_occs.second, m_use_deltas, m_all_correlations);
        // ---- set dformation_energy --------------
